<template>
  <div class="CreateAddress">
   	 	<div class="ErollHead">
	         ...
	         <span class="ErollFont">新增收货地址</span>
    	</div>
			...
			<ul class="userMsg-ul">
				<li>
					<ul class="userMsg-ul-ul">
						<li>
							<span class="msg-li">收货人</span><input type="text" v-model="msg.name" class="fir-input" maxlength="10" ref="getThingPerson"  @focus="blur(1)">
						</li>
						<li>
							<span class="msg-li">手机号码</span><input type="text" v-model="msg.tel" maxlength="11" ref="getThingPhone"  @focus="blur(2)">
						</li>
						<li>
							<span class="msg-li">所在地区</span><span class="atAddress" @click="gotoCityChoose()" ref="atAddress">{{msg.province}}{{msg.city}}{{msg.area}}</span>
						</li>
						<li>
							<span class="msg-li">详细地址</span><input type="text" v-model="msg.address" maxlength="50" ref="getThingAddress" @focus="blur(3)">
						</li>
					</ul>
				</li>
			</ul>
    	...
    	<div class="footer">
  			<div class="f-child" @click="createAddress()"> 
  				保存
  			</div>
    	</div>
 </div>
...
export default {
  ...
  methods:{
    ...
    createAddress(){
    	// 对第一个input进行判断
    	if(this.msg.name.replace(/\s+/g,"")==""){
    		
    		this.$refs.getThingPerson.style.border="1px solid #f40";
    		return false;
    	}

    	// 对第二个input进行判断
    	let dMatch=this.msg.tel.match(/^\d+$/g);
    	dMatch?dMatch=dMatch[0]:dMatch="";
    	
    	if(this.msg.tel.replace(/\s+/g,"")==""||(!dMatch)||dMatch.length!=11){
    		// console.log(123)
    		this.$refs.getThingPhone.style.border="1px solid #f40";
    		return false;
    	}

    	// 对第三个input进行判断
    	if(this.msg.address.replace(/\s+/g,"")==""){
    		
    		this.$refs.getThingAddress.style.border="1px solid #f40";
    		return false;
    	}
    	...
    	if(this.$store.state.token){
    		...
    		this.$axios.post("http://123.206.202.40:82/index.php/api/user/addMyAddress",{
    			token:this.msg.token,
    			name:this.msg.name,
    			address:this.msg.address,
    			tel:this.msg.tel,
    			isdefault:this.msg.isdefault,
    ...
    blur(who){
    	switch(who){
    		case 1:
    		this.$refs.getThingPerson.style.border="1px solid transparent";
    		// console.log(1)
    		break;
    		case 2:
    		this.$refs.getThingPhone.style.border="1px solid transparent";
    		console.log(2)
    		break;
    		case 3:
    		this.$refs.getThingAddress.style.border="1px solid transparent";
    		break;
    	}
  ...
  mounted(){
  	this.$refs.getThingPerson.focus();
  	this.arrCity=city;
